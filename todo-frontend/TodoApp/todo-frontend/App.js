  ...
  try {
    ...
    if (!match) {
      ...
      if (!isNaN(date.getTime())) {
        console.log(`Parsed ISO date: ${dateString} -> ${date.toISOString()}`);
        return date;
      }
...
export default function App() {
  ...
  const scheduleLocalNotification = async (
    ...
    try {
      ...
      if (triggerDate <= now) {
        console.error(
          "Cannot schedule notification for past or current time:",
          datetime
        );
  ...
  const fetchTasks = async () => {
    try {
      console.log("Fetching tasks from:", `${API_URL}/all`);
  ...
  const handleAddTask = async () => {
    ...
    try {
      console.log("Sending POST request to:", API_URL, "with task:", newTask);
  ...
  const openUpdateModal = (task) => {
    ...
    const dateObj = parseTaskDate(task.datetime);
    if (!dateObj) {
      Alert.alert("Error", "Invalid task date. Cannot update task.");
      return;
    }
    ...
    >
      <View style={styles.container}>
        ...
        <FlatList
          ...
          renderItem={({ item }) => {
            const dateObj = parseTaskDate(item.datetime);
            if (!dateObj) {
              console.error("Invalid date for task:", item.datetime);
              ...
              <View style={styles.taskCard}>
                ...
                <View style={styles.buttonGroup}>
                  <TouchableOpacity
                    ...
                    onPress={() =>
                      handleDeleteTask(item._id, item.notificationId)
                    }
                  >
